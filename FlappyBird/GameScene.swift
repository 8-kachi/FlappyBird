...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //衝突判定カテゴリー
    let birdCategory: UInt = 1 << 0     //0...00001
    let groundCategory: UInt = 1 << 1   //0...00010
    let wallCategory: UInt = 1 << 2     //0...00100
    let scoreCategory: UInt = 1 << 3    //0...01000
    ...
    func setupWall() {
        ...
        let createWallAnimation = SKAction.run({
            ...
            scoreNode.physicsBody?.categoryBitMask = UInt32(self.scoreCategory)
            scoreNode.physicsBody?.contactTestBitMask = UInt32(self.birdCategory)
    ...
    func setupBird() {
        ...
        // 衝突のカテゴリー設定
        bird.physicsBody?.categoryBitMask = UInt32(birdCategory)    //追加
        bird.physicsBody?.collisionBitMask = UInt32(groundCategory | wallCategory)    //追加
        bird.physicsBody?.contactTestBitMask = UInt32(groundCategory | wallCategory)    //追加
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (UInt(contact.bodyA.categoryBitMask) & scoreCategory) == scoreCategory || (UInt(contact.bodyB.categoryBitMask) & scoreCategory) == scoreCategory {
            //スコア用の物体と衝突した
            print("ScoreUp")
            score += 1
        } else {
            ...
            bird.physicsBody?.collisionBitMask = UInt32(groundCategory)
